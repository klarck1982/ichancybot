# api/ichancy.py (النسخة النهائية بـ curl_cffi + كوكيز)
...
class IchancyAPI:
    ...
    def _make_request(self, endpoint: str, body: dict):
        ...
        try:
            response = cf_requests.post(
                ...
                auth=auth,
                impersonate="chrome124",   # مهم جداً
                timeout=30
            )
    ...
    async def deposit(self, player_id, amount):
        return self._make_request("/Player/depositToPlayer", {
            "amount": amount,
            "playerId": player_id,
    ...
    async def withdraw(self, player_id, amount):
        return self._make_request("/Player/withdrawFromPlayer", {
            "amount": -abs(amount),
            "playerId": player_id,
            "currencyCode": CURRENCY_CODE,
            "moneyStatus": MONEY_STATUS,
    ...
    async def get_balance(self, player_id):
        return self._make_request("/Player/getPlayerBalanceById", {
            "playerId": player_id
        })

    async def get_statistics(self, start=0, limit=10):
        return self._make_request("/Statistics/getPlayersStatisticsPro", {
            "start": start,
            "limit": limit,
            "filter": {}
        })
